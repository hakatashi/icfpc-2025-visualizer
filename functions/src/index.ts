...
import {info as loggerInfo, error as loggerError} from 'firebase-functions/logger';
...
const ICFPC_API_BASE_URL = 'https://31pwr5t6ij.execute-api.eu-west-2.amazonaws.com';
...
export const exploreAedificium = onCall<ExploreRequest, Promise<ExploreResponse>>(
	async (request) => {
		...
		if (validPlans.length === 0) {
			throw new Error('Invalid request: at least one valid plan is required (digits 0-5 only)');
		}
		...
		try {
			...
			if (!selectResponse.ok) {
				...
				throw new Error(`Problem selection failed: ${selectResponse.status} ${selectResponse.statusText}`);
			}
			...
			if (!exploreResponse.ok) {
				...
				throw new Error(`API request failed: ${exploreResponse.status} ${exploreResponse.statusText}`);
			}
			...
			throw new Error(`Failed to explore aedificium: ${error instanceof Error ? error.message : 'Unknown error'}`);
		}
	}
);
